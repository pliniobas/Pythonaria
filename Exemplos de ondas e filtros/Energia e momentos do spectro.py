...
f = np.random.rayleigh(.1,100)
A = np.random.rayleigh(.4,100) 
...
for ix,ax in enumerate(A):
    onda += np.array([np.sin(aux*2*np.pi * f[ix] + np.random.rand()*0.2*np.pi) * A[ix] for aux in t_series])
...
#%%FFT
#y = np.split(abs(fft(df['filtered_P'])),2)[0]
#x = np.split(fftfreq(len(df['filtered_P']),d = 1/fNy),2)[0]
#dfff = pd.DataFrame(data=dict(x=x,y=y))

#Dividindo o pico maximo pelo numero de ondas que devem ter ocorrido na série
#f_max_energitica = dfff.loc[dfff.y.idxmax(),'x'] #retorna a frequencia do pico maximo.
#temp = tf *  f_max_energitica #numero de ocorrencias na série de frequencia mais energética.

#dfff['y_dividida'] = (dfff['y'] / temp )  

#%% Welch
dx,dy = welch(df.P,fs = fNy,scaling = 'density')
#sx,sy = welch(df.P,fs = fNy,scaling = 'spectrum')#Igual ao de baixo. 
#sxwh,sywh = welch(df.P,fs = fNy,scaling = 'spectrum',window = "hanning") #É igual ao de cima
#sxh,syh = welch(df.filtered_P,fs = fNy,scaling = 'spectrum')

#dffw = pd.DataFrame(data=dict(dx=dx,dy=dy,sx=sx,sy=sy,sxh=sxh,syh=syh,sxwh=sxwh,sywh=sywh))
...
#plt.plot(dfff.x,dfff.y,label='FFT')
#plt.plot(dfff.x,dfff.y_dividida,label='Dividida_FFT')
plt.plot(dffw.dx,dffw.dy,ls='--',label="Welch densidade")
#plt.plot(dffw.sx,dffw.sy,label="Welch raw signal spectrum")
#plt.plot(dffw.sxh,dffw.syh,label="Welch hanningzed out signal")
#plt.plot(dffw.sxwh,dffw.sywh,'.',label="Welch hanningzed welch signal")

Wpower = CubicSpline(dffw.dx,dffw.dy) #Criando uma funcao para calculo de área do spectro
#Wpower = np.poly1d(np.polyfit(dffw.dx,dffw.dy,10))
temp = np.arange(dffw.dx.iloc[0],dffw.dx.iloc[-1],0.001)
plt.plot(temp,Wpower(temp),'-',label='Welch CubicSpline')
plt.legend()

# =============================================================================
# Descobrindo os 3 maiores picos do espectro
# =============================================================================

#Procura o proximo pico maximo a direita de um pico.
def nextpeak(data):
...
for ix,aux in enumerate(x_peaks):
    if len(H3) == 0: #Caso nao haja nenhum valor ainda / primeiro pico
        H3.append(abs(quad(Wpower,aux - Delta_f,aux + Delta_f)[0])**0.5 * 4) 
